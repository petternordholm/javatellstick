 ...
 * @since 1.5.0
 */
public class TellstickDevice implements Comparable<TellstickDevice>,Device{
	...
	/**
	 * Checks if supported methods is set.
	 * This is required for this lib to work.
	 * @throws SupportedMethodsException
	 */
	static public void supportedMethodsCheck() throws SupportedMethodsException{
		if (supportedMethods == -1)throw new SupportedMethodsException("You must set supported methods with TellstickDevice.setSupportedMethods( ... ) to define what methods your application supports.");
	}
	 ...
	 * Can throw exception if failure to create new device.
	 * Note that you must set protocol and arguments.
	 * @throws Exception 
	 * 
	 */
	...
	/**
	 * Returns The right object for this device.
	 * Can be:
	 *  - GroupDevice ( Can have methods from bell, dim and on/off)
	 *  - SceneDevice ( Contains execute )
	 *  - BellDevice
	 *  - DimmableDeviceImpl
	 *  - SwitchableDeviceImpl
	 * @param TS
	 * @param deviceId
	 * @return TellstickDevice , Either BellDevice, DimmableDeviceImpl or SwitchableDeviceImpl.
	 * @throws SupportedMethodsException Exception is thrown if accepted methods is not defined.
	 * @throws DeviceNotSupportedException Exception is thrown if application does not support this device type.
	 */
	static public TellstickDevice getDevice(int deviceId) throws SupportedMethodsException, DeviceNotSupportedException{
		...
		if(type == JNA.CLibrary.TELLSTICK_TYPE_SCENE){
			...
			return new DimmableDeviceImpl(deviceId);
			...
			return new SwitchableDeviceImpl(deviceId);
	...
	static public ArrayList<TellstickDevice> getDevices() throws SupportedMethodsException{
		...
		for (int i = 0; i < nbrDevices; i++) {
			try{
				...
				// SwitchableDeviceImpl not supported. Do not add to list.
			}
		}
		return devices;
	}
	...
	/**
	 * Gets all devices with a filter of instances.
	 * Usage:
	 * ArrayList<TellstickDevice> devices2 = TellstickDevice.getDevices(SwitchableDeviceImpl.class, DimmableDeviceImpl.class, ...etc...);
	 * @param type
	 * @return
	 * @throws SupportedMethodsException
	 */
	@SafeVarargs
	static public ArrayList<TellstickDevice> getDevices(Class<? extends TellstickDevice> ... types) throws SupportedMethodsException {
	    return filterDevices(getDevices(), types);
	}
		
	
	/**
	 * Filters devices into a new list.
	 * Usage:
	 * ArrayList<TellstickDevice> devices2 = TellstickDevice.getDevices(devices, SwitchableDeviceImpl.class, DimmableDeviceImpl.class, ...etc...);
	 * @param allDevices ArrayList of devices for input.
	 * @param type The types , eg DimmableDeviceImpl.class , SwitchableDeviceImpl.class, BellDevice.class, GroupDevice.class ... etc ..
	 * @return
	 * @throws SupportedMethodsException
	 */
	@SafeVarargs
	static public ArrayList<TellstickDevice> filterDevices(ArrayList<TellstickDevice> allDevices, Class<? extends TellstickDevice> ... types) throws SupportedMethodsException {
	...
	/**
	 * Gets a list of TellstickDevice with a supplied list of device Ids in int..
	 * @param deviceIds
	 * @return
	 * @throws SupportedMethodsException 
	 * @throws DeviceNotSupportedException 
	...
	/**
	 * Gets a list of TellstickDevice with a supplied list of device Ids in String.
	 * @param deviceIds
	 * @return
	 * @throws SupportedMethodsException 
	 * @throws DeviceNotSupportedException 
	 ...
	 * Can return
	 * JNA.CLibrary.TELLSTICK_TYPE_GROUP
	 * JNA.CLibrary.TELLSTICK_TYPE_SCENE
	 * JNA.CLibrary.TELLSTICK_TYPE_DEVICE
	 * @return
	 */
	...
	/**
	 * Gets the status of the last action """ CURRENT STATE """ that this device has.
	 * Must be in supported methods.
	 * 
	 * Available statuses:
	 * See setSupportedMethods( ... ), this depends on what the application supports!
	 * 
	 * @return
	 */
	...
	/**
	 * Removes this device totally and deletes it.
	 * Note that the object will still be there in JAVA while in runtime.
	 * @return
	 */
	...
	/**
	 * Tries to learn the device.
	 * Note, must have flag in methods: JNA.CLibrary.INSTANCE.TELLSTICK_LEARN 
	 * @return
	...
	/**
	 * Sets the name of the model of the device.
	 * WARNING; Careful using this!
	 * @param model
	 */
	 ...
	 * Check like:
	 * int methods = JNA.CLibrary.INSTANCE.tdMethods(deviceId, allMethods());
	 * if ((methods & JNA.CLibrary.TELLSTICK_BELL) > 0) // IT HAS BELL ACTION
	 * @return
	 */
